...
up in `list/plausible-sites.txt` to find that site's Plausible script id,
...
To add a site: put `<domain> <script-id>` in `list/plausible-sites.txt` (the
script id is the `pa-…` filename part from Plausible's Site Installation page),
then add the tag above to the site with `?site=<domain>`.
