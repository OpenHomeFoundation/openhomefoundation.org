...
//
// This file is the gate. _data/allowedReferrers.js calls it during the build,
// so an invalid entry in list/allowed-referrers.txt fails the build rather
// than shipping. Adding a dynamic DNS provider or an IP address to the
...
const ALLOW_FILE = join(ROOT, "list", "allowed-referrers.txt");
const DENY_FILE = join(ROOT, "list", "deny-patterns.txt");
...
/**
 * Read, validate and normalise the allowlist.
 *
...
function loadAllowedReferrers() {
  ...
  // exists to enforce.
  const deny = [];
  for (const { lineNumber, raw } of parseListFile(readFileSync(DENY_FILE, "utf8"))) {
  ...
  const seen = new Map();
  for (const { lineNumber, raw } of parseListFile(readFileSync(ALLOW_FILE, "utf8"))) {
